...
This script processes W-2 data for a given year. It downloads files from
the configured Google Drive folder and uses keywords from `config.yaml`
to identify which company and party each PDF belongs to.
...
`data/<year>_her_w2_forms.json`.

Usage:
    python3 extract_w2.py <year> [--cache]
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 extract_w2.py <year> [--cache]")
        print("  year: 4-digit year (e.g., 2024)")
        print("  --cache: Cache downloaded files locally")
        sys.exit(1)
    ...
    if not year.isdigit() or len(year) != 4:
        print(f"Error: Invalid year '{year}'. Must be 4 digits.")
        sys.exit(1)
    ...
    # --- 1. Process all W-2 PDFs ---
    pdf_files = list(source_dir.glob(f"*W-2*.pdf")) + list(source_dir.glob(f"*W2*.pdf"))
    unidentified_pdfs = []
    ...
    # --- 2. Process all Manual W-2 JSON Files ---
    ...
    for manual_path in manual_files:
        ...
        # Use the employer name from the manual file to find company and party from config
        company_info, party = find_company_and_party_from_keywords(employer_name, config)

        if not company_info:
            print(f"Warning: Could not find company config for employer '{employer_name}' in {manual_path.name}. Skipping.")
            continue
